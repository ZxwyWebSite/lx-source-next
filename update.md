...
- `[260418]` 优化 `qdes` 性能，在支持的平台上使用 `cgo` 实现，歌词解密耗时降低至 `1ms` 左右（不含 `unzlib`）
- `[260418]` 更新 内置源数据
- `[260425]` 优化 `wy` 源 `eapi` 性能
- `[260425]` 修复 `tx` 过期时间调整为 `7200` 秒
- `[260523]` 修复 `mg` 链接获取，漏洞又修复了，回退到原始猜路径方式，拿不到试听地址就没法用
- `[260523]` 完善 `lrcutil`，支持分离 `KW` 翻译歌词
- `[260530]` 修复 `kw` 内置 user 被封问题，改为随机生成
- `[260530]` 添加 前端暗色模式支持
- `[260607]` 添加 `bili` 解析源（h5 接口，如需 分 P 在 ID 后加 pN 从 1 开始）
- `[260618]` 缓解 `wy` 账号源获取链接风控问题 `{"code":-460,"message":"检测到您的网络环境存在风险，请稍后再试"}`
- `[260713]` 添加 `wy` 畅听接口 (`AdFree`)，但会员歌曲仅能获取 128k 音质
- `[260714]` 修复 `tx` 取链报错 1000 问题（更换 Web Comm）
- `[260723]` 添加 `kg` 听歌识曲接口
- `[260724]` 优化 `kg` prefetch 流程

### \# 2026-01-24 v1.1.16

- 优化 前端播放列表查询性能、对 IE 的兼容性
- 修复 `KRC` 若 `offset` 不为 `0` 解析失败问题 (`C4904D4E3BBA872D41D3BDBF597E2B18`)
- 其它 更换演示站架构，提升加载速度
- `[260127]` 使用 [`singleflight`](https://zhuanlan.zhihu.com/p/382965636) 彻底解决 **v0** 竞态问题
- `[260208]` 完善 `lrcutil`，将 WY 双冒号歌词 `[00:40:59]` 转换为标准 `[00:40.59]`
...
- 完善 前端播放列表实现（算是吧）
- 优化 部分功能对于 IE 的支持
- 添加 逐字歌词嵌入 `/api/v0/lrc/{source}/{id}/?jmp&lx(&raw)`

### \# 2026-01-02 v1.1.14

- 🎉 祝大家 2026 元旦快乐 🎉 <!-- ，在新的一年里... -->
- 修复 CORS 跨域问题
- 修复 前端密钥传参问题
- 优化 前端播放器媒体信息显示
...
- 重构 前端构建流水线，[开源](https://github.com/lxsrcs/webui)，支持仅前端部署并自定义接口地址
- 添加 前端播放列表实现（这段时间净写这个了）
- 优化 搜索接口本地缓存

### \# 2025-11-29 v1.1.13

- 同步 内置 `Toubiec` 接口修改
- 优化 搜索接口实现，因冲突移除 path 传递 `:pg/:sz` 方式
- 优化 测试版 SSE 日志推送实现
- 添加 LX-Music 播放列表查看器 (`/lxmc`)，之后或许基于此开发本地列表功能
- 添加 测试版 滚动日志实现

### \# 2025-11-22 v1.1.12

- 修复 `KW` 某些未打轴小众歌词报错 `"illegal base64 data at input byte 0"` 问题
- 优化 前端播放器异步请求逻辑
- 添加 `KW` 歌曲付费信息解析（仅供展示）
- 支持 前端播放器提取 `WY` 与 `KW` 的音乐 ID (不含短链) 👿
- 其它 移除部分不常用架构构建
- 优化 `KW` 使用 HTTPS 播放链接（解决无法在 HTTPS 页面播放问题）
- 修复 `menu` 与 flag 冲突问题，改为 `-m` 进入菜单
